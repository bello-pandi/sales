...
class User {
    ...
    addUser(req, res) {
        ...
        try {
            ...
            connectionPool.query(`INSERT IGNORE into sales.user(name, mobile, password, company_name, 
    ...
    loginUser(req, res) {
        try {
            ...
            connectionPool.query(`SELECT id,password FROM sales.user where email = ?`, 'com',
                    ...
                    if (error) {
                        ...
                        if (result.length > 0) {
                            ...
                            if (compareSync(password, result[0].password)) {
                                ...
                                connectionPool.query(`UPDATE sales.user SET auth_token = ? , expiry_time = (SELECT DATE_ADD(NOW(), INTERVAL 24 HOUR)) WHERE id = ?`, [token, result[0].id], function (error2, result2, fields2) {
                                    if (error) {
                                        res.status(500).send({
                                            status: "error",
                                            code: error.code,
     ...
     */
    logoutUser(req, res) {
        connectionPool.query(`UPDATE sales.user SET auth_token = ? WHERE id = ?`, ["", req.user_id], function (error2, result2, fields2) {
            if (error2) {
                res.status(500).send({
                    status: "error",
                    code: error2.code,
    ...
    authenticateToken(req, res, next) {
        ...
        try {
            connectionPool.query(`SELECT id, expiry_time FROM sales.user where auth_token = ?;`, token, function(error, result, fields) {
                if (error) {
                    res.status(500).send({
                        status: "error",
                        code: error.code,
